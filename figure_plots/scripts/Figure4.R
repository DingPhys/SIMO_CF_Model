#!/usr/bin/env Rscript
# Figure4: read the existing CSVs in ../Figure4 and save SVG/PDF figures there.
# Run: Rscript figure_plots/scripts/Figure4.R (from the package root).
...
figure_dir <- normalizePath(file.path(dirname(script_file), "..", "Figure4"))
...
local({
  ...
  cofactor_species <- read.csv(
    file.path(figure_dir, "cofactor_species_order.csv"),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )$species

  binary_frame <- read.csv(
    file.path(figure_dir, "nongrower_binary_CR.csv"),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  ...
  parameter_frame <- read.csv(
    file.path(figure_dir, "cofactor_and_lag.csv"),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  ...
  observed_frame <- read.csv(
    file.path(figure_dir, "double_spent_growth.csv"),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  ...
  save_figure(base_path, function() print(figure), 208.363625 / 72, 207.414158 / 72,
    pdf_width = 208.979 / 72, pdf_height = 208.333 / 72)
...
local({
  ...
  # raw values are left unchanged.

  input_path <- file.path(figure_dir, "ld_panel_plot_source.csv")
  plot_data <- read.csv(
    input_path,
    check.names = FALSE,
    stringsAsFactors = FALSE
  ...
  save_figure(output_base, function() print(figure), 208.363625 / 72, 207.414158 / 72,
    pdf_width = 208.979 / 72, pdf_height = 208.333 / 72)
...
local({
  ...
  bt_cofactor_metrics <- read_table(
    file.path(figure_dir, "bt_cofactor_on_community_metrics.csv")
  )
  bt_resource_metrics <- read_table(
    file.path(figure_dir, "bt_cofactor_off_community_metrics.csv")
  )
  ...
  bt_observed <- read_table(
    file.path(figure_dir, "bt_observed_relative.csv")
  )
  ...
  dm_cofactor_metrics <- read_table(
    file.path(figure_dir, "dm_cofactor_on_community_metrics.csv")
  )
  dm_resource_metrics <- read_table(
    file.path(figure_dir, "dm_cofactor_off_community_metrics.csv")
  )
  ...
  carbon_cofactor_metrics <- read_table(
    file.path(figure_dir, "carbon_cofactor_on_metrics.csv")
  )
  carbon_resource_metrics <- read_table(
    file.path(figure_dir, "carbon_cofactor_off_metrics.csv")
  )
  ...
  save_figure(base_path, draw_figure, 237.310021 / 72, 239.757316 / 72,
    pdf_width = 237.66874 / 72, pdf_height = 239.748 / 72)
...
local({
  ...
  bt_observed <- read_species_matrix(file.path(figure_dir, "bt_observed_relative.csv"))
  bt_predicted <- read_species_matrix(file.path(figure_dir, "bt_cofactor_on_relative.csv"))
  dm_observed <- read_species_matrix(file.path(figure_dir, "dm_observed_relative.csv"))
  dm_predicted <- read_species_matrix(file.path(figure_dir, "dm_cofactor_on_relative.csv"))
  ...
  # R grid lwd = 1 is 0.75 pt in SVG output.
  ...
  save_figure(output_base, draw_figure, figure_width_in, figure_height_in)
