...
# Draw Figure S2 with the established R style and freshly prepared inputs.
# Run through rebuild_figure_s2.py. Final exports are SVG and PDF only.
input_dir <- Sys.getenv("FIG_S2_INPUT")
output_dir <- Sys.getenv("FIG_S2_OUTPUT")
preview_dir <- Sys.getenv("FIG_S2_PREVIEW")
if (!dir.exists(input_dir) || !dir.exists(output_dir)) stop("Missing input/output directory.")
...
consumption_matrix <- read_species_matrix(
  file.path(script_dir, "full_32_CR_matrix.csv")
)
production_matrix <- read_species_matrix(
  file.path(script_dir, "full_32_PR_matrix.csv")
)
...
dm_resource_table <- read.csv(
  file.path(script_dir, "dm_resource_y0.csv"),
  check.names = FALSE,
  stringsAsFactors = FALSE
)
...
bt_resource_table <- read.csv(
  file.path(script_dir, "bt_resource_Y0.csv"),
  check.names = FALSE,
  stringsAsFactors = FALSE
)
...
ragg::agg_png(
  file.path(preview_dir, paste0(basename(output_base), ".png")),
  width = width_in,
  height = height_in,
  units = "in",
  res = 150,
...
message("Species: ", n_rows, "; resources: ", n_cols)
if (length(dropped_resources) > 0L) {
  message(
    "Dropped resources at or below 1.5e-3: ",
    paste(dropped_resources, collapse = ", ")
  )
}
message(
  "Display order: ", length(dm_columns), " DM resources, then ",
  length(bt_columns), " Bt-produced resources"
)
message(
  "Consumption range: ", min(consumption_matrix), " - ", max(consumption_matrix)
)
message(
  "Production range: ", min(production_matrix), " - ", max(production_matrix)
)
message("Saved: ", output_base, ".[svg|pdf]")
})
...
if (length(file_arg) != 1L) {
  stop("Run this file with Rscript so its figure-relative paths can be resolved.")
}

script_path <- normalizePath(sub("^--file=", "", file_arg), mustWork = TRUE)
figure_s2_dir <- input_dir
data_path <- file.path(
  figure_s2_dir,
...
if (!requireNamespace("svglite", quietly = TRUE)) {
  stop("The R package 'svglite' is required.")
}
if (!requireNamespace("ragg", quietly = TRUE)) {
  stop("The R package 'ragg' is required.")
}
...
histogram_source <- data.frame(
  bin_lower = error_histogram$breaks[-length(error_histogram$breaks)],
  bin_upper = error_histogram$breaks[-1],
  count = error_histogram$counts,
  mean_error = mean_error,
  display_cap = y_axis_max,
  n_assemblies = n_assemblies
)
write.csv(
  histogram_source,
  paste0(output_stem, "_histogram_source_data.csv"),
  row.names = FALSE
)
width_mm <- 183
height_mm <- 90
...
message("Wrote figure files with stem: ", output_stem)
message(
  "Assemblies: ", n_assemblies,
  "; species: ", n_species,
  "; error range: ",
  format(min(errors), digits = 5), " to ", format(max(errors), digits = 5),
  "; mean error: ", format(mean_error, digits = 7)
)
})
